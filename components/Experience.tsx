...
const education = {
  ...
  recognition: '1st place in War Room Debate at Banglalink HQ (2023), organized by IEEE BRACU CS SBC',
};
...
export default function Experience() {
  ...
  return (
    <section id="experience" className="relative py-32 bg-aot-dark-wall overflow-hidden" ref={ref}>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        >
          <div className="glass-card rounded-2xl p-8 relative overflow-hidden group">
            <motion.div
              className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-aot-green/20 to-transparent rounded-bl-full"
            ...
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6 mb-6">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-aot-cream">{education.degree}</h3>
                    <p className="text-aot-green font-medium">{education.institution}</p>
                  </div>
                </div>
              </div>
              <div className="text-right">
                <div className="text-aot-green font-bold">{education.period}</div>
                <div className="text-sm text-aot-smoke">{education.location}</div>
              </div>
            </div>

            <div className="mb-4">
              <h4 className="text-sm font-bold text-aot-cream uppercase tracking-wider mb-3">Relevant Coursework</h4>
            ...
            >
              <p className="text-aot-smoke text-sm">
                <span className="text-aot-green font-semibold">Recognition: </span>
        ...
        <div className="relative">
          {/* Timeline Line */}
          <motion.div
            className="absolute left-4 md:left-1/2 md:-translate-x-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-aot-green via-aot-green-dim to-transparent"
            initial={{ scaleY: 0 }}
            ...
            {experiences.map((exp, index) => (
              ...
              >
                ...
                <motion.div
                  ...
                  whileHover={{ scale: 1.5, boxShadow: '0 0 20px rgba(45, 212, 191, 0.8)' }}
                />
                  ...
                  >
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex-1">
                        ...
                        <p className="text-aot-green font-medium">{exp.company}</p>
                      </div>
                      <div className="text-right text-sm">
                        <div className="text-aot-green font-bold whitespace-nowrap">{exp.period}</div>
                    ...
                    <ul className="space-y-2">
                      {exp.description.map((desc, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm text-aot-smoke">
                    ...
                    <div className="mt-4">
                      <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${
                        ...
                        {exp.type === 'work' ? '⚔️ Work' : exp.type === 'education' ? '📚 Education' : '👑 Leadership'}
                      </span>
                    </div>
                  </motion.div>
                </div>
