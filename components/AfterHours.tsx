...
import { FaFilm, FaTv, FaBook, FaMicrophone, FaNewspaper, FaMask } from 'react-icons/fa';
...
const categories = ['All', 'Movie', 'Series', 'Anime', 'Book', 'Podcast', 'Article'];
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        >
          ...
          <p className="text-gray-600 max-w-2xl mx-auto font-medium mb-8">
            When the scouts return from the walls, we seek refuge in stories, music, and knowledge. 
        ...
        >
          {categories.map((cat) => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className={`flex items-center gap-2 px-6 py-2.5 rounded-sm text-sm font-black uppercase tracking-tighter transition-all duration-300 border-3 border-black ${
                activeCategory === cat
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredData.map((item, index) => (
            <motion.div
              ...
              animate={isInView ? { opacity: 1, rotate: index % 2 === 0 ? -1 : 1, y: 0 } : {}}
        ...
        >
          <p className="font-black text-black uppercase tracking-tighter text-xl">
            &quot;Those who cannot abandon anything, can never change anything.&quot;
          </p>
          <p className="text-aot-red font-bold mt-2">— Armin Arlert</p>
        </motion.div>
      </div>
    </section>
  );
